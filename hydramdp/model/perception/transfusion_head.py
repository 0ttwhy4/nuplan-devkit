...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, query, key, query_pos, key_pos, attn_mask=None):
        ...
        if not self.cross_only: # perform self attention first if not cross_only
        ...
        query2 = self.multihead_attn(query=self.with_pos_embed(query, query_pos_embed),
                                     ...
                                     attn_mask=attn_mask)[0] # note that in cross attention the key and value are usually the same
        ...
        return query # the query mixed with information from k and v
...
class TransFusionHead(nn.Module):
    def __init__(self,
                 ...
                 dropout=0.1,
                 bn_momentum=0.1,
                 # the update momentum of mean value used in bn
                 activation='relu',
                 # config for FFN
                 common_heads=dict(),
                 num_heatmap_convs=2,
        ...
        if not self.use_sigmoid_cls:
            self.num_classes += 1 # In sigmoid classification, each class is assigned with a logit (instead of probability) of the 
                                  # object belonging to the given class, and hence no "background class" is needed since the model can
                                  # directly assign low logit for all classes to indicate background class. 
